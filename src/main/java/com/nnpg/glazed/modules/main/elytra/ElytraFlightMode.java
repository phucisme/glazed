...
public class ElytraFlightMode {
    ...
    private Vec3 fireworkVelocity = Vec3.ZERO;
    ...
    public void onActivate() {
        ...
        ticksLeft = 0;
        acceleration = 0;
        fireworkBoostTicks = 0;
        fireworkVelocity = Vec3.ZERO;
    }
    ...
    private void updateFireworkBoost() {
        if (!elytraFly.useFireworks.get() || !elytraFly.autoPilot.get() || !mc.player.isFallFlying()) {
            fireworkBoostTicks = 0;
            fireworkVelocity = Vec3.ZERO;
            return;
        }
        if (ticksLeft > 0) {
            ...
            fireworkBoostTicks = 20;
            ticksLeft = elytraFly.autoPilotFireworkDelay.get() * 20;
        }

        if (fireworkBoostTicks > 0) fireworkBoostTicks--;
    }

    void applyFireworkBoost() {
        if (fireworkBoostTicks > 0) {
            Vec3 look = mc.player.getLookAngle();
            fireworkVelocity = fireworkVelocity.add(
                look.scale(0.1).add(look.scale(1.5).subtract(fireworkVelocity).scale(0.5))
            );
        } else {
            fireworkVelocity = fireworkVelocity.scale(0.99);
        }

        velX += fireworkVelocity.x;
        velY += fireworkVelocity.y;
        velZ += fireworkVelocity.z;
    }
